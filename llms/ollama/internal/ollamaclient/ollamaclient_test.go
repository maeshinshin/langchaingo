...
func TestOptionsJSONMarshalWithThink(t *testing.T) {
	...
	// Verify think is at the top level (sibling of model/format), not inside options,
	// and that it serializes as a JSON boolean (not the string "true").
	think, exists := result["think"]
	assert.True(t, exists, "think field should exist in JSON")
	assert.Equal(t, true, think, "think field should be the JSON boolean true")
	assert.NotContains(t, data, []byte(`"options":{"think"`),
		"think must not be nested under options")
	...
	temp, exists := options["temperature"]
